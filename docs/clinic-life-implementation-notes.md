...
Telegram admin -> protected Railway webhook -> deterministic state machine
               ...
               -> Google Sheets News repository -> rolling published window (3)
               -> GET /api/news -> vanilla-JS Clinic Life renderer

Website lead form -> POST /api/leads -> Google Sheets Leads audit row
                  -> Telegram delivery to every configured admin
                  -> success response only after confirmed delivery
```
...
- Publishing is idempotent by `publish_request_id` and serialized by an in-process mutex.
- After every publish/restore, all published rows after the newest three are archived and the repository is re-read and asserted before Telegram receives success.
...
- Backend: Express API, security middleware, Google Sheets repository, Cloudinary media adapter, Telegram client/CMS state machine, news/lead services, safe schema initializer, webhook setup script, and Railway config under `server/`.
